...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [vRes, kRes, rRes, prevRes] = await Promise.all([
        supabase
          .from("vendedores")
          .select(
      ...
      const firstError =
        vRes.error ?? kRes.error ?? rRes.error ?? prevRes.error;
      if (firstError)
        setError(
  ...
  const filteredKpis = useMemo(
    () =>
      selectedKpiId === "todos"
        ? kpis
        : kpis.filter((kpi) => kpi.id === selectedKpiId),
    [kpis, selectedKpiId],
  );

  const metrics = useMemo(() => {
    const activeKpiCount = filteredKpis.length;
    ...
    const vendorStatus = vendedores.map((vendedor) => {
    ...
    const avg = averageAdvance(vendedores, filteredKpis, registros);
    const previousAvg = averageAdvance(vendedores, filteredKpis, previousRegistros);
    const delta =
  ...
  }, [registros, vendedores, filteredKpis, previousRegistros]);
  ...
  }, [vendedores, filteredKpis, registros]);
  ...
  const cards = [
    ...
    },
    {
      ...
      value:
        selectedKpiId === "todos"
          ? kpis.length
          : `${filteredKpis.length} de ${kpis.length}`,
      icon: Target,
      href: "/admin/kpis",
      tone: "border-slate-200 bg-white",
    },
