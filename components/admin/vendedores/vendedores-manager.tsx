...
export function VendedoresManager() {
  ...
  async function save() {
    ...
    const { error } = editing
      ? await supabase.from("vendedores").update(payload).eq("id", editing)
    ...
    <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
      <Card>
        <CardContent className="space-y-4 pt-5">
          ...
          <div className="flex gap-2"><Button onClick={save}>{editing ? <Save className="h-4 w-4" /> : <Plus className="h-4 w-4" />} Guardar</Button>{editing ? <Button variant="outline" onClick={() => { setEditing(null); setForm(empty); }}>Cancelar</Button> : null}</div>
        </CardContent>
      </Card>
