...
import { advanceLabel, advanceTone, calcPercent, formatPercent, todayInLima } from "@/lib/utils";
...
export function ReportesClient({ mode }: { mode: "reportes" | "exportar" }) {
  ...
  const loadBase = useCallback(async () => {
    ...
    const [vRes, kRes, sRes] = await Promise.all([
      ...
      supabase.from("supervisores").select("id,usuario_id,codigo_operativo,nombre,activo,created_at").eq("activo", true).order("codigo_operativo")
    ]);
    const firstError = vRes.error ?? kRes.error ?? sRes.error;
  ...
  const supervisorVendedores = useMemo(
    () => vendedores.filter((item) => supervisorId === "todos" || item.jefe_id === supervisorId),
    [supervisorId, vendedores],
  );

  const supervisorKpis = useMemo(
    () => kpis.filter((item) => supervisorId === "todos" || item.jefe_id === supervisorId),
    [kpis, supervisorId],
  );

  const zonas = useMemo(() => [...new Set(supervisorVendedores.map((v) => v.zona))].sort(), [supervisorVendedores]);

  const filteredVendedores = useMemo(() => supervisorVendedores.filter((v) => {
    if (zona !== "todos" && v.zona !== zona) return false;
    if (vendedorId !== "todos" && v.id !== vendedorId) return false;
    return true;
  }), [supervisorVendedores, vendedorId, zona]);

  const filteredKpis = useMemo(() => supervisorKpis.filter((k) => {
    if (grupo !== "todos" && getKpiGroup(k) !== grupo) return false;
    if (kpiId !== "todos" && k.id !== kpiId) return false;
    return true;
  }), [grupo, kpiId, supervisorKpis]);
  ...
  const supervisorByUserId = useMemo(
    () => new Map(supervisores.map((item) => [item.usuario_id, item])),
    [supervisores],
  );

  const reportSummary = useMemo(() => {
    const compromiso = avanceRows.reduce((total, row) => total + row.compromiso, 0);
    const cierre = avanceRows.reduce((total, row) => total + row.cierre, 0);
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardContent className="grid min-w-0 gap-3 pt-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-9">
          ...
          <Select value={supervisorId} onChange={(event) => { setSupervisorId(event.target.value); setZona("todos"); setVendedorId("todos"); setKpiId("todos"); setGrupo("todos"); }} aria-label="Supervisor">
            <option value="todos">Todos los supervisores</option>
            {supervisores.map((supervisor) => <option key={supervisor.usuario_id} value={supervisor.usuario_id}>{supervisor.codigo_operativo} · {supervisor.nombre}</option>)}
          </Select>
          ...
          <Select value={vendedorId} onChange={(event) => setVendedorId(event.target.value)} aria-label="Vendedor"><option value="todos">Todos los vendedores</option>{supervisorVendedores.map((v) => <option key={v.id} value={v.id}>{v.nombre}</option>)}</Select>
          <Select value={grupo} onChange={(event) => setGrupo(event.target.value as KpiGroupName | "todos")} aria-label="Grupo KPI"><option value="todos">Todos los grupos</option>{getOrderedGroupNames(supervisorKpis).map((item) => <option key={item} value={item}>{item}</option>)}</Select>
          <Select value={kpiId} onChange={(event) => setKpiId(event.target.value)} aria-label="KPI"><option value="todos">Todos los KPI</option>{supervisorKpis.map((k) => <option key={k.id} value={k.id}>{k.nombre}</option>)}</Select>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Card>
          <CardContent className="pt-5">
            ...
            <p className="mt-1 text-3xl font-black text-slate-950">{reportSummary.cierre.toLocaleString("es-PE")}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            <p className="text-sm font-bold text-slate-500">Alcance %</p>
            <p className="mt-1 text-3xl font-black text-blue-700">{formatPercent(reportSummary.alcance)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {loadingBase || loading ? (
        ...
        <div className="kpi-scrollbar max-h-[72vh] w-full max-w-full overflow-auto rounded-2xl border bg-white shadow-soft">
          <table className="min-w-[1160px] w-full text-sm">
            <thead className="sticky top-0 z-10"><tr className="border-b bg-slate-100"><th className="p-3 text-left">Fecha</th><th className="p-3 text-left">Supervisor</th><th className="p-3 text-left">Zona</th><th className="p-3 text-left">Vendedor</th><th className="p-3 text-left">KPI</th><th className="p-3 text-right">Compromiso</th><th className="p-3 text-right">Corte</th><th className="p-3 text-right">Cierre del día</th><th className="p-3 text-center">Alcance %</th><th className="p-3 text-left">Estado</th></tr></thead>
            <tbody>{avanceRows.map((row) => {
              const vendedor = filteredVendedores.find((item) => item.id === row.vendedor_id);
              const supervisor = vendedor ? supervisorByUserId.get(vendedor.jefe_id) : undefined;
              return <tr key={`${row.fecha}-${row.vendedor_id}-${row.kpi_id}`} className="border-b hover:bg-blue-50/50"><td className="p-3">{row.fecha}</td><td className="p-3 font-semibold">{supervisor ? `${supervisor.codigo_operativo} · ${supervisor.nombre}` : "—"}</td><td className="p-3 font-bold">{row.zona}</td><td className="p-3">{row.vendedor}</td><td className="p-3">{row.kpi}</td><td className="p-3 text-right">{row.compromiso}</td><td className="p-3 text-right">{row.corte}</td><td className="p-3 text-right">{row.cierre}</td><td className="p-3 text-center"><span className={`rounded-full border px-2 py-1 text-xs font-black ${advanceTone(row.avance)}`}>{advanceLabel(row.avance)}</span></td><td className="p-3">{row.estado}</td></tr>;
            })}</tbody>
          </table>
        </div>
      )}
