...
import type { Etapa, Kpi, RegistroKpi, Supervisor, Vendedor } from "@/types/database";
...
export function TablaExcel({ readOnly = false }: { readOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!supervisores.some((item) => item.usuario_id === supervisorId)) {
  ...
  useEffect(() => {
    ...
    vendedores.forEach((currentVendedor) => {
      kpis
        .filter((currentKpi) => currentKpi.jefe_id === currentVendedor.jefe_id)
        .forEach((currentKpi) => {
          etapas.forEach((etapa) => {
            nextDrafts[makeCellKey(currentVendedor.id, currentKpi.id, etapa.key)] = "0";
          });
        });
    });
    ...
    setCellErrors({});
  }, [fecha, kpis, registros, vendedores]);
  ...
  const rows = useMemo(() => buildAvanceRows({ fecha, vendedores: scopedVendedores, kpis: scopedKpis, registros }), [fecha, scopedVendedores, scopedKpis, registros]);
  const zonas = useMemo(() => [...new Set(scopedVendedores.map((item) => item.zona))].sort(), [scopedVendedores]);

  const visibleVendedores = useMemo(() => scopedVendedores.filter((item) => {
    ...
    return true;
  }), [estado, rows, scopedVendedores, search, vendedor, zona]);

  const visibleKpis = useMemo(() => scopedKpis.filter((item) => {
    ...
    return true;
  }), [grupo, kpi, scopedKpis, showAdicionales]);
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="grid min-w-0 gap-3 pt-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-9">
          ...
          <Select value={supervisorId} onChange={(event) => { setSupervisorId(event.target.value); setZona("todos"); setVendedor("todos"); setKpi("todos"); setGrupo("todos"); }} aria-label="Supervisor"><option value="" disabled>Selecciona supervisor</option>{supervisores.map((item) => <option key={item.usuario_id} value={item.usuario_id}>{item.codigo_operativo} · {item.nombre}</option>)}</Select>
          ...
          <Select value={vendedor} onChange={(event) => setVendedor(event.target.value)} aria-label="Vendedor"><option value="todos">Todos los vendedores</option>{scopedVendedores.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</Select>
          <Select value={grupo} onChange={(event) => setGrupo(event.target.value as KpiGroupName | "todos")} aria-label="Grupo KPI"><option value="todos">Todos los grupos</option>{getOrderedGroupNames(scopedKpis).map((item) => <option key={item} value={item}>{item}</option>)}</Select>
          <Select value={kpi} onChange={(event) => setKpi(event.target.value)} aria-label="KPI"><option value="todos">Todos los KPI</option>{scopedKpis.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</Select>
          ...
          <div className="flex gap-2">
            ...
            {!readOnly ? <Button type="button" variant={viewMode === "detalle" ? "default" : "outline"} className="flex-1" onClick={() => setViewMode("detalle")}>Detalle editable</Button> : null}
          </div>
          ...
          {viewMode === "detalle" && !readOnly ? <Button onClick={savePendingChanges} disabled={loading || savingAll || dirtyKeys.size === 0} className="xl:col-span-2"><Save className="h-4 w-4" /> {savingAll ? "Guardando..." : `Guardar cambios${dirtyKeys.size ? ` (${dirtyKeys.size})` : ""}`}</Button> : null}
      ...
      {readOnly ? <Alert tone="info">Vista de gerente: puedes consultar todos los supervisores y reportes, pero no modificar registros.</Alert> : null}
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500">
        ...
        <span>{readOnly ? "Solo aparecen vendedores y KPI marcados como “Mostrar en Tabla Excel”. Esta vista es exclusivamente de consulta." : "Solo aparecen vendedores y KPI marcados como “Mostrar en Tabla Excel”. En Detalle editable puedes corregir Compromiso, Corte y Cierre; luego presiona Guardar cambios."}</span>
      </div>
